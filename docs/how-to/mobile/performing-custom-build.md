...
The standard ONNX Runtime inferencing [build instructions](../build/inferencing#build-instructions) apply, with some additional options being specified to reduce the binary size.
...
The follow options can be used to reduce the build size:
  ...
  - [REQUIRED] Enable the minimal build (`--minimal_build`)
  ...
  - [REQUIRED] Reduce build to required operator kernels
    - Add `--include_ops_by_config <config file produced during model conversion> --skip_tests` to the build parameters.
    - See the documentation on the [Reduced Operator Kernel build](../build/reduced) for more information. 
  ...
  - [OPTIONAL] Reduce types supported by the required operators
    - To enable operator type reduction, add `--enable_reduced_operator_type_support` to the build parameters.
        - NOTE: Requires ONNX Runtime version 1.7 or higher and for type reduction to have been enabled during model conversion

  - [OPTIONAL] Disable exceptions (`--disable_exceptions`)
  ...
  - [OPTIONAL] Disable ML operator support (`--disable_ml_ops`)
  ...
  - [OPTIONAL] Use shared libc++ on Android (`--android_cpp_shared`)
