...
Install the onnxruntime-mobile Android package from ... **FIXME**

### iOS

Install the onnxruntime-mobile CocoaPods package from ... **FIXME**
  - TODO: 
    - Check name of pod. 
    - What else is required here? 
    - Is there also an option to install a lower level iOS package with the C/C++ API is someone wants to use that?
...
You must match the python package version to the branch of the ONNX Runtime repository you checked out
  - e.g. if you wanted to use the 1.7 release 
    - checkout the `rel-1.7.2` branch in the git repository
