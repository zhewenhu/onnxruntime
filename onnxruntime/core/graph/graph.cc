...
Status Graph::PerformTopologicalSortAndCheckIsAcyclic() {
  ...
  std::unordered_set<NodeIndex> downstream_nodes;  // nodes producing output that has been consumed by nodes_seen entries
  ...
  // find all the leaf nodes (nodes with no output edges as there's no edge to a graph outputs)
